...
// src/git_tools/parser.ts
var parseDiff = (rawDiff) => {
...
var IGNORED_FILES = ["package-lock.json", "yarn.lock", "pnpm-lock.yaml", ".DS_Store"];
var clearDiffContent = (content) => {
  return content.split(`
`).filter((line) => {
    const iscodeChange = line.startsWith("+") || line.startsWith("-");
    const isCodeMetadata = line.startsWith("+++") || line.startsWith("---") || line.startsWith("diff --git") || line.startsWith("index ") || line.startsWith("@@");
    return iscodeChange && !isCodeMetadata;
  }).join(`
`);
};
var filterChanges = (Changes) => {
...
`;
    const content = change.content.length > 1000 ? change.content.slice(0, 1000) + `
...
// src/git_tools/commit.ts
import { exec as exec2 } from "child_process";
import { promisify as promisify2 } from "util";
var execAsync2 = promisify2(exec2);
var GitCommit = async (message) => {
  try {
    const { stdout } = await execAsync2(`git commit -m ${JSON.stringify(message)}`);
    return stdout;
  } catch (error2) {
    console.error("Error while git commit ", error2);
    return "";
  }
};
...
program2.name("git-ai-commit").description("AI-powered commit message generator").version("1.0.0");
program2.command("diff").description("shows git diff").action(async () => {
  const diff = await getStagedDiff();
  if (!diff) {
    console.log(source_default.yellow("no staged changes found! first try 'git add .' or 'git add ./filename'"));
    process.exit(1);
  }
  console.log(diff);
});
program2.command("filter-diff").description("show the filtered diff.").action(async () => {
  const diff = await getStagedDiff();
  if (!diff) {
    console.log(source_default.yellow("no staged changes found! first try 'git add .' or 'git add ./filename'"));
    process.exit(1);
  ...
  console.log(source_default.yellow(`filter changes : `));
  console.log(changes);
});
program2.command("aicommit").description("shows the prased file chnage difference").action(async () => {
  validateConfig();
  const diff = await getStagedDiff();
  if (!diff) {
    console.log(source_default.yellow("no staged chnages found! first try 'git add .' or 'git add ./filename' "));
    process.exit(1);
  }
  const changes = filterChanges(parseDiff(diff));
  ...
  if (!message) {
    console.log(source_default.red("Error while generating message!"));
    process.exit(1);
  }
  console.log(source_default.yellow("Proposed Commit Message:"));
  console.log(message + `
`);
  const commitRes = await GitCommit(message);
  console.log(source_default.green.bold("commited following, Now push your changes!"));
